...
4. 很多第三方端点**不支持 responses WebSocket**，Codex 先试 WS（404/405）再回退 HTTPS，每次多 ~5-12s，但功能正常。
...
## 注意
- 桌面版每次请求约 5-12s WebSocket 重试延迟（第三方/代理多无 WS），暂无干净办法关闭（内置 openai provider 不可覆盖）。
- 旧会话线程可能保留旧模型/认证选择，务必开新对话验证。
- CC Switch 代理模式下，实际端点和 key 在 CC Switch 数据库的 `providers` 表 `settings_config` 字段中管理，
  不需要手动改 `auth.json`。
- 想零延迟且稳定，建议改用 **Codex CLI**（终端 `codex`），它读顶层 `base_url` + apikey 模式直接生效。
